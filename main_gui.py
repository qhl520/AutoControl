...
class AutoControlApp:
    def __init__(self, root):
        self.root = root
        self.root.title("SISO 自动控制系统设计平台 Pro v2.0")
    ...
    def create_sidebar(self):
        ...
        ttk.Label(
            title_frame, 
            text="⚡ SISO自动控制器设计平台 v2.0", 
            font=("微软雅黑", 14, "bold"), 
            foreground='#2980b9'
        ).pack(side=LEFT)

        # ========== 1. 被控对象模型（极致压缩内边距+外边距） ==========
        group_plant = ttk.Labelframe(self.left_panel, text="🏭 被控对象模型", padding=8)
        group_plant.pack(fill=X, pady=(0, 6))  # 大幅减少模块间距
        ...
        self.btn_run.pack(fill=X, ipady=3)  # 减少按钮高度
        ...
        # ========== 6. 日志输出区【核心放大】：抢占左侧剩余所有空间 ==========
        log_frame = ttk.Labelframe(self.left_panel, text="📝 设计日志（核心放大）", padding=8)
        # 关键配置：fill=BOTH+expand=YES → 占满所有剩余空间，实现最大化
        log_frame.pack(fill=BOTH, expand=YES, pady=(5, 0))
        self.txt_log = scrolledtext.ScrolledText(
            log_frame, font=("Consolas", 9), wrap=tk.WORD,
            relief=tk.FLAT, bg="#f8f9fa", bd=0
        )
        self.txt_log.pack(fill=BOTH, expand=YES)  # 日志框自适应填满父容器

    def create_labeled_entry(self, parent, label_text, default_val, hint_text=""):
        """紧凑版输入框组件：减少内边距、缩小字号，节省空间"""
        container = ttk.Frame(parent)
        container.pack(fill=X, pady=(0, 4))  # 减少输入框间距
